...
export default function CommunionsListPage() {
  ...
  const years = useMemo(() => {
    ...
    communions.forEach((c) => {
      if (c.communionDate?.length >= 4) set.add(c.communionDate.slice(0, 4));
    });
  ...
  return (
    <AuthenticatedLayout>
      <div className="space-y-6">
        {/* Title + Add Communion (desktop: both; mobile: title only, Add at bottom) */}
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-8 shadow-sm text-center">
              <p className="text-gray-600">
                {communions.length === 0
                  ...
                  : 'No communions match the current filters.'}
              </p>
              {communions.length === 0 && (
                <Link
                  href={`/communions/new?parishId=${parishId}`}
                ...
                <li key={c.id}>
                  <div className="flex items-start gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                    ...
                    >
                      ...
                      <span className="block text-sm text-gray-600 mt-1">{c.communionDate}</span>
                      ...
                      {c.officiatingPriest && (
                        <span className="block text-xs text-gray-500 mt-0.5">{c.officiatingPriest}</span>
                      )}
                    </button>
